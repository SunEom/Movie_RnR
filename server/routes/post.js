...
router.get('/', async function (req, res, next) {
  await db.query(`SELECT movie.id, title, overview, movie.created, genres, rates, movie.updated, user_id, count(comment.id) FROM movie LEFT JOIN comment ON movie.id=comment.movie_id GROUP BY movie_id ORDER BY movie.created desc LIMIT 20;`, 
